...
$sfname = 'server/php/files/'.$_POST["sfname"].'.zip';
unzip($sfname);
$xfile='server/php/files/'.$_POST["xfile"].'.xml';
$rpname= $_POST["rpname"];
...
for ($i = 0; $i < $nofPackages; $i++) {
    ...
    for ($j = 0; $j < sizeof($classes); $j++) {
        ...
        for ($k = 0; $k < sizeof($methods); $k++) {
            $methodsValues = array(
                array('name' => $methods[$k]->attributes()->name, 'ccn' => $methods[$k]->attributes()->ccn, 'ccn2' => $methods[$k]->attributes()->ccn2, 'cloc' => $methods[$k]->attributes()->cloc, 'eloc' => $methods[$k]->attributes()->eloc, 'lloc' => $methods[$k]->attributes()->lloc, 'loc' => $methods[$k]->attributes()->loc,
        ...
        $classesValues = array(
            array('name' => $classes[$j]->attributes()->name, 'ca' => $classes[$j]->attributes()->ca, 'cbo' => $classes[$j]->attributes()->cbo, 'ce' => $classes[$j]->attributes()->ce, 'cis' => $classes[$j]->attributes()->cis, 'cloc' => $classes[$j]->attributes()->cloc,
    ...
    $packagesValues = array(
        array('package_name' => $packages[$i]->attributes()->name,
...
for ($i = 0; $i < $nofFiles; $i++) {
    ...
    $filesValues = array(
        array('file_name' => $file_name, 'full_path' => $files[$i]->attributes()->name, 'cloc' => $files[$i]->attributes()->cloc,
